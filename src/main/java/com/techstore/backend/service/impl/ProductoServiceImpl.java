...
public class ProductoServiceImpl implements ProductoService {
    ...
    public byte[] exportarExcel(Boolean estado) throws IOException {
        ...
        String[] cols = {"ID", "Nombre", "DNI", "Telefono", "Email", "Estado"};
            ...
            for (Producto p : lista) {
                ...
                row.createCell(1).setCellValue(nvl(p.getNombre()));
                row.createCell(2).setCellValue(nvl(p.getDescripcion()));
                row.createCell(3).setCellValue(nvl(p.getCategoria()));
                row.createCell(4).setCellValue(nvl(p.getEmail()));
                row.createCell(5).setCellValue(Boolean.TRUE.equals(p.getEstado()) ? "Activo" : "Inactivo");
            }
    ...
    public byte[] exportarPDF(Boolean estado) throws Exception {
        ...
        table.setWidths(new float[]{1f, 3f, 2f, 2f, 3f, 2f});
        ...
        for (String h : new String[]{"ID", "Nombre", "DNI", "Telefono", "Email", "Estado"}) {
    ...
    public int importarDesdeCSV(MultipartFile archivo) throws IOException {
        ...
        for (int i = 1; i < lines.length; i++) {
            ...
            String campo1 = clean(cols[0]); // nombre
            String campo2 = cols.length > 1 ? clean(cols[1]) : ""; // DNI
            String campo3 = cols.length > 2 ? clean(cols[2]) : ""; // telefono
            String campo4 = cols.length > 3 ? clean(cols[3]) : ""; // email

            if (campo1.isBlank() || productoRepository.existsByNombreIgnoreCase(campo1)) continue;

            Producto p = new Producto();
            p.setNombre(campo1);
            p.setDescripcion(campo2.isBlank() ? null : campo2);
            p.setCategoria(campo3.isBlank() ? null : campo3);
            p.setEmail(campo4.isBlank() ? null : campo4);
            p.setEstado(true);
    ...
    public int importarDesdeExcel(MultipartFile archivo) throws IOException {
        ...
        try (Workbook wb = WorkbookFactory.create(archivo.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                String campo1 = cell(row, 0); // nombre
                String campo2 = cell(row, 1); // DNI
                String campo3 = cell(row, 2); // telefono
                String campo4 = cell(row, 3); // email
